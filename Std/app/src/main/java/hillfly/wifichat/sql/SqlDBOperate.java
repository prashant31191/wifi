...
public class SqlDBOperate {
    ...
    private Message chattingInfoToMessage(ChattingInfo chattingInfo) {
        ...
        message.setMsgContent(chattingInfo.getInfo()); // Set up聊天信息内容
        message.setSendTime(chattingInfo.getDate());// Set up聊天信息发送时间
        switch (chattingInfo.getStyle())// Set up聊天信息类型
        {
            case 0:
        ...
        message.setSenderIMEI(getIMEIByUserID(chattingInfo.getSendID()));// Set up发送方ID
        return message;
    }
