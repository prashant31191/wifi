...
public class WifiapAdapter extends BaseAdapter {
    ...
    // æ–°åŠ çš„ä¸€ä¸ªå‡½æ•°ï¼Œç”¨æDEFAULT¥æ›´æ–°æ•°æDEFAULT®
    ...
    private String getDesc(ScanResult ap) {
        ...
        if (ap.SSID.startsWith(WifiApConst.WIFI_AP_HEADER)) {
            //desc = "ä¸“ç”¨ç½‘ç»œï¼ŒåDEFAULT¯ä»¥ç›´æŽ¥è¿žæŽ¥";
        	desc = "This is app user network";
        }
        else {
            String descOri = ap.capabilities;
            if (descOri.toUpperCase().contains("WPA-PSK")
                    || descOri.toUpperCase().contains("WPA2-PSK")) {
                //desc = "åDEFAULT—åˆ°å¯†ç DEFAULTä¿DEFAULTæŠ¤";
            	desc = "Secured network";
            }
            else {
                //desc = "æœªåDEFAULT—ä¿DEFAULTæŠ¤çš„ç½‘ç»œ";
            	desc = "Unsecure network";
            	
            }
        }

        // æ˜¯åDEFAULT¦è¿žæŽ¥æ­¤çƒ­ç‚¹
        if (isWifiConnected) {
            //desc = "å·²è¿žæŽ¥";
        	desc = "Connected";
        }
